...
TypeScript 有 Null 和 Undefined 两种特殊的类型，他们各自的值分别为 `null` 和 `undefined` 。之前我们无法显式得命名这两种类型，但现在我们可以将 `null` 和 `undefined` 作为类型名称来实现类型的推断。

在以前的版本中，类型检测允许将 `null` 和 `undefined` 赋值给任意类型的变量。事实上， `null` 和 `undefined` 可以是*任意*类型的值，我们也不可能特殊得排除掉他们（所以也不可能检测到错误使用他们的情况）。


### `--strictNullChecks`

`--strictNullChecks` 切换到一个新的严格的空类型检测模式。  

在严格的空类型检测模式下， `null` 和 `undefined` 值*不*属于各自类型范围内，他们仅仅可以赋值给他们自己和 `any` 类型（ `undefined` 也可以赋值给 `void` 类型是一种特殊情况）。所以， `T` 和 `T | undefined` 在常规类型检测模式中是等价的（因为 `undefined` 被看做是任一类型 `T` 的子类型），但是在严格类型检测模式中他们两个是不同的，只有 `T | undefined` 类型可以接收 `undefined` 的值。同样的，`T` 和 `T | null` 也是这样的关系。
...
在严格的空类型检测模式中，编译器要求每一个类型中不包含 `undefined` 的变量必须先被赋值然后才可以使用。
...
编译器通过执行 *基于类型分析的控制流* 来检测确保变量被正确的赋值。具体的将会在这节稍后来讲解。

### Optional parameters and properties

可选的参数和属性被自动添加了 `undefined` 类型，即使没有明确得在他们的类型注解中包含 `undefined` 。比如，下面的两种类型是等价的：
...
获取一个包含 `null` 或 `undefined` 类型的对象的属性或调用一个包含 `null` 或 `undefined` 类型的函数都会产生一个编译时的错误。但是类型推断支持 non-null 和 non-undefined 类型的检测。 
...
non-null和non-undefined的类型推断可以通过使用 `==`，`!=`，`===` 或 `!==` 运算符来与 `null` 和 `undefined` 做比较，就像 `x != null` 或 `x === undefined`。  
变量类型所产生的效果与原生的 JavaScript 保持语义上的精确对应（e.g. double-equals operators check for both values no matter which one is specified whereas triple-equals only checks for the specified value）。

### Dotted names in type guards

之前类型推断仅支持本地变量和参数。现在类型推断可以检测由一个跟随一个或多个属性入口的变量或参数名“点命名（dotted names）”。
